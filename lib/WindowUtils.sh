...
fluxion_window_close() {
	...
	if [ "$pid" ]; then
		# In tmux mode: find and kill the window whose pane matches pid.
		if [ "$FLUXIONDisplayMode" = "tmux" ]; then
			local window
			for window in $(tmux list-windows -F '#{window_name}:#{pane_pid}' 2>/dev/null); do
				local wName="${window%%:*}"
				local wPid="${window##*:}"
				if [ "$wPid" = "$pid" ] || pgrep -P "$wPid" 2>/dev/null | grep -qx "$pid"; then
