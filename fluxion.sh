...
fluxion_shutdown() {
  ...
  [ -n "${authService:-}" ] && kill -s SIGKILL $authService &> $FLUXIONOutputDevice || true
...
fluxion_do_sequence() {
  ...
  while [ "$__fluxion_do_sequence__instruction" ]; do
    ...
    if ! fluxion_do $__fluxion_do_sequence__namespace $__fluxion_do_sequence__instruction; then
      ...
      (( __fluxion_do_sequence__instructionIndex++ )) || true
    fi
...
fluxion_set_language() {
  ...
  # Use process substitution (not pipe) so 'return 1' propagates to the outer function.
  local _langMissing=0
  ...
  if [ "$_langMissing" -eq 1 ]; then # If a file is missing, fall back to english.
...
fluxion_get_target() {
  ...
  if [ "$FLUXIONAuto" ]; then
    ...
    if [[ "$__ifBands" == *"6GHz"* ]]; then
      choices+=("$FLUXIONScannerChannelOptionAll (6GHz - Wi-Fi 6E)")
    fi
    if [[ "$__ifBands" == *"2.4GHz"* ]] && [[ "$__ifBands" == *"5GHz"* ]]; then
      choices+=("$FLUXIONScannerChannelOptionAll (2.4GHz & 5Ghz)")
    fi
    if [[ "$__ifBands" == *"2.4GHz"* ]] && [[ "$__ifBands" == *"5GHz"* ]] && [[ "$__ifBands" == *"6GHz"* ]]; then
      choices+=("$FLUXIONScannerChannelOptionAll (All Bands)")
    fi
    ...
    case "$IOQueryChoice" in
      ...
      "$FLUXIONScannerChannelOptionAll (6GHz - Wi-Fi 6E)")
        fluxion_target_get_candidates $interface "" "ax";;

      "$FLUXIONScannerChannelOptionAll (2.4GHz & 5Ghz)")
        fluxion_target_get_candidates $interface "" "abg";;

      "$FLUXIONScannerChannelOptionAll (All Bands)")
        fluxion_target_get_candidates $interface "" "abgx";;
      ...
      "$FLUXIONScannerChannelOptionSpecific")
        ...
        # Channels 1-14 are 2.4GHz (band bg), 36-177 are 5GHz (band a)
        # 6GHz Wi-Fi 6E channels are 1-233 in 6GHz spectrum (airodump uses 'ax')
        local band=""
        local firstChannel=$(echo "$channels" | grep -oE '[0-9]+' | head -1)
        if [ -n "$firstChannel" ]; then
          if [ "$firstChannel" -le 14 ]; then
            band="bg"
          elif [ "$firstChannel" -le 177 ]; then
            band="a"
          else
            band="ax"  # 6GHz Wi-Fi 6E
          fi
        fi
  ...
  for candidateAPInfo in "${FluxionTargetCandidates[@]}"; do
    ...
    elif [ "$__ch" -ge 149 -a "$__ch" -le 177 ] 2>/dev/null; then
      # 6GHz channels (Wi-Fi 6E) — mark with 6G suffix for visibility
      candidatesChannel[i]="${__ch}₆"
    else
      candidatesChannel[i]="$__ch"
    fi
